...
int crioSetSTRINGOUTItem(struct crio_context *ctx, const char *name, char value[40]) {
    ...
    try {
        ...
        if (is_rt_var(name) == true)
        {
            ...
            set_rt_str(ctx->shared_memory, address, value);
            if (ctx->debugCRIO)
            {
                printf ("RT STRINGOUT name=%s, Value=%s, Index=%d\n" , name, value, address);
                fprintf (ctx->log, "RT STRINGOUT name=%s, Value=%s, Index=%d\n" , name, value, address);
            }
        }
